...
import { MapPin, DollarSign, Clock, Star, Snowflake, Globe, Loader2 } from "lucide-react"
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const res = await fetch("/api/trips?activity=skiing")
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-8 text-center">
            <p className="text-muted-foreground">
              Found <span className="text-primary font-semibold">{filteredActivities.length}</span> skiing trips
            </p>
          </div>
          ...
          {isTripsLoading ? (
            ...
            <div className="text-center py-16">
              <p className="text-muted-foreground text-lg">No skiing trips found. (Admin can add trips in Admin → Trips)</p>
            </div>
          )}
        </div>
      </section>
